...
class Filter(nn.Module):
    ...
    def Kernel_gen(self, Archi, kernelInit, kernTrainable, Style, transpose=False):
        ...
        if transpose == False:
            K_lp = torch.tensor(kernelInit, dtype=torch.float32)
            K_hp = torch.multiply(torch.flip(torch.tensor(kernelInit, dtype=torch.float32), [2]),
                self.cmf)
        else:
            K_lp = torch.flip(torch.tensor(kernelInit, dtype=torch.float32), [2])
            K_hp = torch.multiply(torch.tensor(kernelInit, dtype=torch.float32),
                                  self.cmf*-1)
    ...
    def Biases_gen(self, Archi, initHT, trainHT):
        ...
        if Archi == "WPT":
            for i in range(self.level):
                ...
                for j in range(int(2**i)):
                    biases.append(nn.Parameter(
                        data=torch.tensor(
                            np.array([initHT])[np.newaxis, np.newaxis, :].copy(), dtype=torch.float32).clone(),
            ...
            for i in range(self.level):
                ...
                biases.append(nn.Parameter(
                    data=torch.tensor(
                        np.array([initHT])[np.newaxis, :].copy(), dtype=torch.float32).clone(),
                    requires_grad=trainHT))
                biases.append(nn.Parameter(
                    data=torch.tensor(
                        np.array([initHT])[np.newaxis, :].copy(), dtype=torch.float32).clone(),
